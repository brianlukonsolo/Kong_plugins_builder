...
| `debug_log_saml_request` | `true` | Logs the outgoing `SAMLRequest` value and AuthnRequest XML. Disable outside local debugging. |
...
| `debug_capture_dir` | `/kong-saml-debug` | Container path mounted to host `saml-plugin-outputs` for full SAMLRequest and SAMLResponse capture files. |
